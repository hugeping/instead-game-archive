...
-- Принято.]];
	...
	"SOS", а также передаёт показания приборов. Возможно, это поможет
	комиссии Национального совета по безопасности разобраться в
	причинах аварии и избежать подобного в будущем.]]
				else
					p [[Ты уже
